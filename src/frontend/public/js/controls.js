import * as corr from "./vis/correlation.js";
import * as args from "./vis/barchart.js";
import * as exts from "./vis/2d-scatterplot.js";
...
function init() {
    ...
    // specific controls for the tools

    args.controls();
    exts.controls();
    corr.controls();

    document.getElementById("undo-selection").addEventListener("click", function () {
        document.dispatchEvent(new CustomEvent('undo-hl', { detail: new Set(["arguments", "extensions", "correlation"]) }));
    });


    document.getElementById("new-project").addEventListener("click", function() {
        M.Modal.getInstance(document.getElementById('new-project-modal')).open();
    })
}
